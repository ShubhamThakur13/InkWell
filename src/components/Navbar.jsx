...
const Navbar = () => {
   ...
   let {Mode, setMode, LoginUser} = useContext(MyBlogData);
   console.log(LoginUser);
   
  return (
    <div className='flex items-center justify-around p-3 border-b absolute w-screen max-w-[1600px] m-auto'>
        <div className="logo">
        ...
        <div className="Auth flex items-center gap-4">
            ...
            {LoginUser ? <div onClick={()=>{}} className='flex gap-2 items-center hover:bg-[#008B74] hover:px-2 hover:py-1 hover:rounded-xl hover:text-white'>
                <p className='p-1 px-3 bg-[#0056A4] text-white rounded-3xl'>{LoginUser.name.slice(0,1)}</p>
                <p>{LoginUser.name}</p>
            </div> :   <div className="buttons flex gap-2">
                <button onClick={()=> {
                    navigate('/login')
